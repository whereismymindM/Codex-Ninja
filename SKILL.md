...
| assets/board-templates/ | 标准化公告牌模板集（调查轮×7/待命/收工/老渣对讲/流程体验/自检——复制改名 `board_NNN.md` 即用） |
| scripts/compose.js + scripts/templates/ | 编排自动化：JSON 声明式生成公告牌+状态矩阵+编译期流转校验（20 个现实团队流程模板，`node compose.js --list` 查看） |
