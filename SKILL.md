...
- 你在旁边窗口盯场救火、看产出、陪故国有明聊天（盯场视图：`node <skill路径>/scripts/ecoscope.js <项目根目录>` 一页看角色存活/轮次进度/签字产出矩阵）
...
| 3 | 时序校验 | 跑 `bash <skill路径>/scripts/时序校验.sh <项目根目录>`（`<skill路径>` = 本仓库路径）——自动查三种模式顺序违规（双人抢答/辩论跳步/审核打回异常），mtime 判定零成本 | 文件 mtime + 操作日志 |
...
| 6 | 迭代 Skill | 分析调查反馈（协作/时间/大鱼调查）→ 改模板 → **改完跑 `node <skill路径>/scripts/doc-consistency.js` 自查漂移（用前读 <skill路径>/scripts/doc-consistency-使用说明.md）** → 重建角色跑下一轮 | — |
...
| scripts/check.js | 收工核对工具（老渣待办 #2 自动化）——收工后全链路只读校验 5 项（发布一致性/产出/签字/退场/两件套），判据与 monitor 同源 |
| scripts/ecoscope.js | 生态仪表盘（盯场视图，共识 P0/P1 第二位）——运行中随时跑：角色存活心跳表（静默下线一眼可见）+ 轮次进度矩阵 + 告警汇总，只读不写文件；心跳阈值与 monitor 同源 |
