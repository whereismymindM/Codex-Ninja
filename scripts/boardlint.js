    ...
    b.roles.forEach(function(r) {
      ...
      if (n === boards[0].n) {
        ...
        if (pa !== undefined && !prevIsRetire) {
          ...
          // 2026-08-31 补强: 全链比对——上轮本轮后(明确写) 应等于 本轮状态（"本轮后:活跃" 但下轮 "状态:待命" = 对不上）；跳过收工轮（退场终局，本轮后不比对）
          if (pa !== "休眠" && pa !== "退场" && pa !== st && b.mode !== "收工") {
            warns.push("第" + pad3(n) + "轮 角色 '" + r.name + "' 状态流转不一致: 上轮本轮后='" + pa + "' 但本轮状态='" + st + "'（本轮后应对应下轮状态：本轮后:活跃 则下轮应 状态:活跃；本轮后:待命 则下轮应 状态:待命）");
          }
        }
      }
