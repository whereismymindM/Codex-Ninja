#!/bin/bash
# sequence_check.sh —— 收工后老渣待办 #3：mtime 顺序合规检查
# 用法: bash sequence_check.sh <项目根目录>
# 检查三种协作模式的流程顺序是否合规（靠文件 mtime 判定，零 token，无需 AI 汇报）：
#   双人对话：问先答后（答.md mtime >= 问.md）
#   辩论：01立论->02立论->03找茬->04找茬->05自由辩论->06总结（mtime 单调递增）
#   主笔审核：please-review mtime <= review-result mtime（打回循环交替递增）
# 输出：✅ 合规 / ⚠️ 违规（含文件名与时间）
# 退出码：0=全部合规 1=发现违规 2=用法错误
# 2026-08-07 初版（ASCII基线 后设计，收工两件套配套）

if [ -z "$1" ]; then
    echo "用法: bash sequence_check.sh <项目根目录>"
    echo "例:   bash sequence_check.sh \"<项目根目录绝对路径>\""
    exit 2
fi
WORLD="$1/world"
if [ ! -d "$WORLD" ]; then
    echo "ERROR: 找不到 world/（$WORLD）——请确认传入的是项目根目录"
    exit 2
fi
...
while IFS= read -r qfile; do
    FOUND=1
    # 从问文件推导对应答文件
    afile="${qfile%_ask.md}_answer.md"
    if [ -f "$afile" ]; then
        ...
        if [ "$at" -lt "$qt" ]; then
            ...
            LOG "✅ $(basename "${qfile%_ask.md}") 顺序正确（问 $(date -d @$qt +%H:%M:%S) → 答 $(date -d @$at +%H:%M:%S)）"
        fi
    fi
done < <(find "$WORLD" ! -path "*_回收站*" -name "chat_*_T*_ask.md" 2>/dev/null | sort)
[ "$FOUND" = "0" ] && LOG "  （无双人对话文件）"
...
while IFS= read -r bdir; do
    ...
    files=$(find "$bdir" -maxdepth 1 -name "debate_*.md" ! -name "debate-end.md" 2>/dev/null | while read f; do echo "$(TS "$f") $f"; done | sort -n | awk '{print $2}')
    if [ -z "$files" ]; then continue; fi
    # 终结收敛豁免（2026-08-08 修复）：任务目录存在 debate-end.md = 任一方主动终结，
    # 之后直接进入 06 总结（跳过多余回合）——05→06 的"阶段倒退"是合法收敛，不报违规。
    term_file="$bdir/debate-end.md"
    if [ -f "$term_file" ]; then
        term_ts=$(TS "$term_file")
        LOG "  📌 $(basename "$bdir") 有debate-end.md（$(date -d @$term_ts +%H:%M:%S)）——终结收敛，05 后跳步豁免"
    else
        term_ts=0
    fi
    ...
    for f in $files; do
        ...
        # 提取阶段号（debate_NN_* 的 NN）
        stage=$(echo "$name" | sed -n 's/^debate_\([0-9][0-9]*\)_.*/\1/p')
...
# ── 主笔审核：please-review <= review-result（打回循环配对）──
echo ""
echo "[主笔审核] 每次please-review.md 须早于对应 review-result（打回循环：请审->结果N->重发请审->结果N+1）"
FOUND=0
while IFS= read -r sdir; do
    FOUND=1
    # 配对：please-review.md 与 review-result_第N次.md 按 mtime 就近配对，检查每对 请审 <= 结果
