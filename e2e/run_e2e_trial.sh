...
# 依据：任务003_做肉 e2e_assert.md（后端开发-DHH 设计，T1-T7 规格）
...
# ── 构造沙箱：monitor.js + 我的世界/ ──
setup() { # $1=沙箱路径
  local d="$1"
  mkdir -p "$d/我的世界/产出" "$d/我的世界/任务NNN_试用"
...
cp "$TMPROOT/t1/我的世界/公告牌_001.md" "$S/我的世界/"
mkdir -p "$S/我的世界/任务001_试用"
echo "反馈：请验证 TRIAL 输出" > "$S/我的世界/任务001_试用/试用反馈.md"
OUT=$(cd "$S" && node monitor.js 2>&1)
...
cp "$TMPROOT/t1/我的世界/公告牌_001.md" "$S/我的世界/"
mkdir -p "$S/我的世界/任务001_试用" "$S/我的世界/产出/任务001_试用" "$S/我的世界/测试甲_大鱼对讲"
echo "反馈" > "$S/我的世界/任务001_试用/试用反馈.md"
echo "OK" > "$S/我的世界/产出/任务001_试用/试用报告.md.ready"
echo "sign" > "$S/我的世界/测试甲_大鱼对讲/完成_001.md"
OUT=$(cd "$S" && node monitor.js 2>&1)
...
echo "{\"N\":1,\"waitSinceN\":1,\"waitSince\":$PAST_MS}" > "$S/我的世界/.monitor_state.json"
OUT=$(cd "$S" && node monitor.js 2>&1)
echo "── T4 等反馈阶段熔断豁免 ──"
...
- 任务: 全员退场
EOF
mkdir -p "$S/我的世界/测试甲_大鱼对讲"
touch "$S/我的世界/测试甲_大鱼对讲/测试甲已退场_001"
OUT=$(cd "$S" && node monitor.js 2>&1)
echo "── T7 收工轮回归 ──"
