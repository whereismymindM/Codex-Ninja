...
printf '[{"name":"testA","desc":"solo test","background":"test role"}]' > roles.json
OUT=$(node "$SKILL/scripts/scaffold.js" "$T" roles.json 2>&1)
check "角色 reasonix.toml" "testA/reasonix.toml" "$OUT"
check "大鱼 reasonix.toml" "fish/reasonix.toml" "$OUT"
check "monitor.js 生成" "monitor.js" "$OUT"
check "玩法文件占位符=0" "0" "$(grep -c '{{ROLE_NAME}}' testA/_solo_output_mode.md)"
check "_sign.js 占位符=0" "0" "$(grep -c '{{ROLE_NAME}}' testA/_sign.js)"
...
cd testA
OUT=$(node _reasonix_poll.js testA 0); check "BULLETIN N=1" "BULLETIN N=1" "$OUT"

echo "===== 4. 角色干活：产出 + deliver + sign ====="
mkdir -p ../world/output/task001_测试 ../world/testA_talk
printf '# 测试报告\n内容：e2e 验证\n' > ../world/output/task001_测试/报告.md
OUT=$(node _deliver.js 报告.md task001_测试); check "DELIVERED" "DELIVERED" "$OUT"
...
check "testA未退场 MISS" "RETIRE testA MISS" "$OUT"

echo "===== 6. 角色 poll 收工轮 + 创建退场文件 ====="
cd testA
node _reasonix_poll.js testA 1 >/dev/null 2>&1
RC=$?
touch ../world/testA_talk/testAretired_002
...
cat > world/board_003.md <<'EOF'
# 公告牌 第003轮
- 模式: 待命
- testA（状态：待命，本轮后：待命，等通知）
- 任务: 全员待命等通知
- 产出: world/output/task003_待命/x.md
EOF
OUT=$(node monitor.js)
