...
bash run_e2e_monitor.sh              # monitor 机制（已签未交点名 / 产出计数真实 / DONE 文案全量 / 推进）
bash run_e2e.sh <skill路径>          # 指定 skill 路径（含空格请加引号）
```

> 总断言：14 + 19 + 19 + 9 + 17 + 7 = **85**（2026-09-01 新增 run_e2e_monitor.sh 7 断言，含复检路径 DONE 文案真断言）
...
| 5. monitor 推进 | 001 完成 → 002 收工轮，未退场 → `RETIRE testA MISS` + `WAIT N=2`（正确不误判） |
...
- **中文角色名**：内置「testA/testB/testC」，顺带验证中文路径/命名
...
**流程**：改 Skill → 跑五脚本（14+19+19+9+17=78 断言）→ 全绿再提交。
