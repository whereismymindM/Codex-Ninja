...
mkdir -p fish world/output/task001_测试 world/架构师_talk
...
- 模式: 单人输出
- 架构师（状态：活跃，本轮后：待命）
- 任务: 写测试文档到 world/output/task001_测试/out.md
- 产出负责人: 架构师
- 产出: world/output/task001_测试/out.md
...
- 任务: 全员退场
EOF
OUT=$(node "$SKILL/scripts/boardlint.js" fish 2>&1)
...
mkdir -p 异常
cat > 异常/board_001.md <<EOF
# 公告牌 第001轮
$FIRST
- 模式: 交叉碰撞
- 架构师（状态：活跃，本轮后：待命）
- 任务: 写 {角色}_报告.md 到 world/output/task001_测试/
- 产出负责人: 架构师
- 产出: world/output/task001_测试/{角色}_报告.md
- 任务目录: world/task001_测试/
EOF
cat > 异常/board_003.md <<EOF
# 公告牌 第003轮
$FIRST
- 模式: 收工
- 架构师（状态：退场）
...
# ── 3. check：收工批次应全绿（output/签字/退场/两件套）──
echo "===== 3. check 收工批次（全绿 exit 0）====="
cp fish/board_001.md world/
cp fish/board_002.md world/
echo "测试文档内容" > world/output/task001_测试/out.md
echo "size: 24" > world/output/task001_测试/out.md.ready
...
rm world/架构师_talk/done_001.md world/架构师_talk/架构师retired_002
OUT=$(node "$SKILL/scripts/check.js" . 2>&1); RC=$?
...
echo "$(date +%s%3N)" > world/架构师_talk/_heartbeat.txt
