...
### ecoscope.js（生态仪表盘，盯场用）

```bash
node scripts/ecoscope.js <项目根目录>
# 例：node scripts/ecoscope.js "一号舱室-软件开发部"（项目根 = 含 我的世界/ 与 火影-大鱼/ 的目录）
```

- **运行中随时跑**（共识 P0/P1 第二位，生态工具清单）——一页盯场视图（只读、不写文件）：角色存活表（心跳 mtime + 新鲜度 + 判定，静默下线一眼可见）+ 轮次进度矩阵（签字/产出含 producer 归属）+ 告警汇总
