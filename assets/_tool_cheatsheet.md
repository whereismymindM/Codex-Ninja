...
- 心跳阈值与 monitor 同源（窗口常驻 2min / run 拉起 10min，读 `_run_shape.mode`）；退场角色心跳停标记"已退场（正常）"不误报
...
- ⚠️ **唤醒边界（2026-08-22）**：`_wakeup.md` 只对**正在轮询**的角色有效——**休眠态**可唤醒；**待命态**不用唤醒（poll 到新牌自动活跃）；**回合结束/窗口崩溃**（进程不轮询）→ 写信号无效，**只能用户**在角色窗口输入对话/重开窗口。**老渣也是 agent**——和大鱼一样只能写文件信号，不能替用户往窗口输入；遇到回合结束/崩溃态角色，老渣的职责 = **把大鱼发来的求助转告用户**（大鱼会写 `fish_laozha_talk/fish-chat_NNN.md` 求助老渣，老渣盯场时查看 `world/fish_laozha_talk/`），不是自己调 `_wakeup.js`（写了也没人读）。**判定**：角色 poll-log（`world/{角色名}_talk/{角色名}_poll-log.md`）在更新=在轮询（可唤醒）；停了=不在轮询（只能用户）——回合结束与窗口崩溃证据相同，判不了也不需判。
