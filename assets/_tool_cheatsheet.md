...
- ⚠️ **唤醒边界（补充）**：`_wakeup.md` 只对**正在轮询**的角色有效——休眠可唤醒；待命不用（自动活跃）
- 回合结束/窗口崩溃：写信号无效，**只能用户**在角色窗口输入对话/重开窗口
- 老渣也是 agent（只能写文件）——此态唯一路径 = 大鱼写求助老渣转告用户；不能以文字结束回合"提醒"用户
- 判定：角色 poll-log 在更新=在轮询（可唤醒）；停=不在轮询（只能用户）；两态证据相同不需判
