...
- ⚠️ **唤醒边界（2026-08-22）**：`_wakeup.md` 只对**正在轮询**的角色有效——**休眠态**可唤醒；**待命态**不用唤醒（poll 到新牌自动活跃）；**回合结束/窗口崩溃**（进程不轮询）→ 写信号无效，**只能用户**在角色窗口输入对话/重开窗口。**老渣也是 agent**——和大鱼一样只能写文件信号，不能替用户往窗口输入；遇到回合结束/崩溃态角色，老渣的职责 = **提醒用户去窗口唤醒**，不是自己调 `_wakeup.js`（写了也没人读）。
