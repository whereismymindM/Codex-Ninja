...
> ⚠️ **日常用独立脚本，本文件不用读！** 以下内联 JS 函数**仅作 fallback**——独立脚本（`_sign.js`/`_deliver.js`/`_lock.js`）不满足需求（需要定制内容/逻辑）时才手抄。**默认动作永远是：`node _sign.js N` / `node _deliver.js 文件 任务NNN` / `node _lock.js acquire|release`**。跳过本文件不影响任何日常干活。
>
> 独立脚本优先的理由：参数校验 + 原子写 + 自检，免手抄出错面（12-18 角色复盘：内联样板是共同最卡点，每轮手抄 15-40 行 = 出错面）。
