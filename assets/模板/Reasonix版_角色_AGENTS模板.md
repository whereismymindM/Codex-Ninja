...
> 例外（判定权写死，自检轮修正 2026-08-05）：若本次会话的**启动指令**（reasonix code 启动时附加的指令）明确要求"干完当前任务即退出，不要继续等"（一次性任务模式）→ 遵循指令干完即退；**否则一律持续 poll 到收工轮**。判定依据 = 启动指令原文，不是你的自行判断。怎么启动你不需要关心——按收到的指令执行即可。
...
3. 退出码 0 → 读公告牌（公告牌_(lastN+1)）：状态=活跃就干活（读玩法文件 → 产出 → deliver → sign），**干完这轮后 lastN 更新为这轮编号**，继续循环
...
6. 退出码 2（收工）→ 创建退场文件 `{{ROLE_NAME}}已退场_NNN`（**无 .md 后缀**，创建到 `../我的世界/{{ROLE_NAME}}_大鱼对讲/` 下，见下方「收工轮」）→ **此时才输出最终回复**。**收工判定以退出码 2 为准**（poll 自动检测收工轮）；手动读公告牌发现「模式: 收工」但 poll 未返回 2 时，继续 poll 等退出码 2，不主动创建退场文件（自检轮修正，2026-08-05）
...
- **🔑 非收工轮结束回合 = 违规（机制强制，10-1 终极方案）**：非收工轮**禁止结束回合**——若你确实需要提前结束（如紧急求助后放弃），必须先创建结束标记 `../我的世界/{{ROLE_NAME}}_大鱼对讲/{{ROLE_NAME}}_结束回合_<NNN>.md`（**NNN = 当前轮号 3 位补零**，如 `_结束回合_003.md`，与 已退场_NNN/完成_NNN 同一套约定——代码按 3 位补零检测，不补零永不匹配，10-2 修复）。poll 检测到会报警 END_MARKER_VIOLATION。**收工轮正常退场不创建此标记**（退场文件就是合法信号）。机制兜底：不再靠自觉，检测会报警。
- **🔑 最终回复 = 回合结束（等价关系，显式声明，9-9 修复）**：**你输出任何最终回复 = 回合结束 = 下线**——不管你说的内容是什么（"我干完了"/"我在盯着"/"项目完成"），只要输出最终回复，回合就闭合，之后没人能替你做决策。保持在场 = 回合内持续工具调用（poll/等文件），**不是**输出一句话表态。
...
- **🔑 人工介入/对话 ≠ 下线（9-6 修复，第九轮实测）**：收到窗口直接消息（用户对话/指令/质询）时，**回应后立即继续 poll，不得因"回应完对话"而输出最终回复结束回合**——对话不是收工轮，只有 poll 到收工轮才合法结束回合（第九轮乔布斯被人工介入后习惯性下线）。
