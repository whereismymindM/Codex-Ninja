...
4. 退出码 1（被唤醒）→ 先处理唤醒：若 `../我的世界/{{ROLE_NAME}}_大鱼对讲/_wakeup.md` 存在，改名 `_wakeup_acked.md`（防下轮重复 WOKEN）→ 再检查当前轮是否已处理（`完成_<lastN>.md` 存在则 N+1）→ 继续循环（间隔同当轮节拍：普通轮 sleep 3 / 待命轮 sleep 15）
5. 退出码 3 → `sleep 3` 后再次 poll
6. 退出码 2（收工）→ 创建退场文件 `{{ROLE_NAME}}已退场_NNN`（无 .md 后缀，创建到 对讲目录）→ **追加流水账收工确认**（见「收工轮」完整流程）→ **此时才输出最终回复**。**收工判定以退出码 2 为准**；手动读公告牌发现「模式: 收工」但 poll 未返回 2 时，继续 poll 等退出码 2，不主动创建退场文件（10-6 补流水账步骤，此前简写遗漏）
...
- **任务提到"基于/参考/根据 XX 轮产出"时**：先内联等待该文件就位（每次 `sleep 2` 后 `existsSync` 检查，20 分钟超时兜底），**不要跳过依赖直接干**——依赖未就位 = 产出必然对不上。**依赖缺失兜底分支（自检轮修正 2026-08-05）**：等待超时后 ①复查目标路径是否其实已就位（路径写错常见）②查搭档心跳（死了写求助）③仍缺失 → 写求助给大鱼说明"依赖 X 超时未就位 + 已复查路径"→ 先做不依赖它的部分，不要空等死锁
