...
4. 退出码 1（被唤醒）→ 先处理唤醒：若 `../我的世界/{{ROLE_NAME}}_大鱼对讲/_wakeup.md` 存在，改名 `_wakeup_acked.md`（防下轮重复 WOKEN）→ 再检查当前轮是否已处理（`完成_<当前N>.md` 存在则 N+1）→ 继续循环（间隔同当轮节拍：普通轮 sleep 3 / 待命轮 sleep 15）
...
- **普通干活轮**：每轮 poll 一次即可，**不要**用 while 阻塞等待（bash 等待期间你看不到输出，阻塞 = 无法干活）；**待命/休眠轮例外**——必须用下方 while 持续 poll 模板（待命轮询 bash 循环）保持在场（自检轮修正 2026-08-05）
...
- **待命轮之后**：持续 poll 等通知——新公告牌来了就干活，收工轮来了就退场。**只有收到收工轮（`模式: 收工`，半角冒号+空格，与 :144 标准写法一致）才允许停止**
...
4. **先确认退场文件已创建（第 2 步完成），才输出「项目完成」**——顺序不可颠倒；此时输出最终回复 = 回合结束（合法收尾点）。**不要再 poll 等下一张牌**（第八轮实测教训：图灵退场后继续 poll 等不存在的 007，浪费 token 且无意义——追加由老渣主动发起，不需要你退场后继续等；收工轮之后没有新牌属正常）
5. 停止循环——这是唯一可以停下来的时刻
...
> **8-3 自检轮修正（2026-08-05）已全部并入本文**——非活跃轮跳过的是干活不是轮次推进、收工判定以退出码2为准、CWD铁律、续心跳量化、wait_file优先、20分钟超时继续poll 等（10-7 补可见标注：此前角色检索"8-3"字面找不到）。
>
...
| 1 | 访问我的世界用 `../我的世界/`（相对角色目录），别切工作目录；**所有 `_*.js` 工具（_reasonix_poll/_deliver/_sign/_lock/_poll/wait_file）必须在角色目录 CWD 下跑——禁止 cd 进 临时脚本/ 或子目录再调用**（CWD 偏移 = 相对路径全错 + 心跳断，第九轮乔布斯事故教训；9-1 起 wait_file.js 已内部锚定，其他工具仍靠本条约束） |
