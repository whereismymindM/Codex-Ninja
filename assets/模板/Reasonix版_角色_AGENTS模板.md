...
- **🔑 非收工轮结束回合 = 违规（机制强制，10-1 终极方案）**：非收工轮**禁止结束回合**——若你确实需要提前结束（如紧急求助后放弃），必须先创建结束标记 `../我的世界/{{ROLE_NAME}}_大鱼对讲/{{ROLE_NAME}}_结束回合_<当前N>.md`（poll 检测到会报警 END_MARKER_VIOLATION）。**收工轮正常退场不创建此标记**（退场文件就是合法信号）。机制兜底：不再靠自觉，检测会报警。
