...
### BUG 13：Codex 桌面闪退（磁盘 I/O 过载）

**现象**：两个角色同时 poll 公告牌时，Codex 桌面突然关闭（不报错、不留日志）。
**根因**：_poll.js 频繁读写磁盘，两个进程并发 → I/O 饱和 → Codex Electron 进程被系统杀掉。
**发生条件**：经典模式，2+ 角色窗口同时 active poll。
**规避方案**：
- _poll.js 初始间隔不低于 5 秒（当前 10 秒已经够安全）
- 遇到闪退后重开窗口即可恢复（公告牌和产出文件不受影响）
- 如果频繁闪退，考虑把两个角色的 poll 间隔错开（一个 10s，一个 13s）
