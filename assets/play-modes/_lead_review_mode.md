...
**2. 发please-review**
- **单审核方**：在任务目录下写 `please-review.md`，告诉搭档可以审了。写完立即写 `please-review.md.signal`（信号文件，内容统一 `{文件名} 已就位`——只是存在性标记）。
...
写完立即写 `review-result.md` 同名 `.signal`（多审核方对应 `review-result_{角色名}.md.signal`；信号内容统一 `{文件名} 已就位`——只是存在性标记）（信号文件通知），后面写具体内容：
