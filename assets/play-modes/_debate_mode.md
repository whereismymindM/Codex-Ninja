...
**写文件方**：写完辩论文件后，立即在同目录写同名的 `.signal` 文件（如 `debate_01_pro-stmt.md.signal`），内容为时间戳（仅存在性标记——读方不读内容，只认文件是否存在）。
...
> 🔑 **裁判掉线接手（正反方视角）**：正反方**签完字后不再等裁判**（签字独立于裁判交付）。裁判掉线由 monitor 兜底（产出卡轮 30 分钟 → WAIT_OVERDUE 报警 + 写 `needs-intervention`）。
