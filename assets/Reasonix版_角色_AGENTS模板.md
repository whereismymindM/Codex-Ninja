...
公告牌写 `模式: 待命`（或你的状态=待命）时触发本流程。这轮没你的事，不用签字。不许说告退/再见/done，不许关窗口。
...
3. **禁止把"继续待命轮询"当最终回复输出**：turn 驱动下，无工具调用的最终回复 = turn 结束 = 静默下线。说的不算，做的才算——要么真的在循环，要么明说"本轮结束，等外部唤醒"，不许说"继续轮询"然后退出。
