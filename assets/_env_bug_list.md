...
| **MSYS 路径翻译** | 工具脚本路径参数一律相对路径（`../world/`），禁止 POSIX 绝对路径（`/tmp` 会被改写成 `D:\tmp`） |
...
| 3 | **路径一律相对** | `../world/`，禁止 POSIX 绝对路径（MSYS 会改写） | MSYS 路径翻译 |
...
**规避**：
- **统一用相对路径**（`../world/...`）——相对路径不以 `/` 开头，MSYS 不碰
- 确需绝对路径：用盘符形式（`C:/x/y`），或 `JSON.stringify(path)` 注入
- **铁律**：工具脚本路径参数一律相对路径（`../world/`），禁止 POSIX 绝对路径——与 `_workflow.md`「路径与临时文件」节 路径锚定铁律 同口径

---

## 6. 写temp-scripts：用 write_file 原生直写

**现象**：用 heredoc / `cat >` 写含中文/反引号/正则的临时 JS 文件时，引号转义出错 → 内容被破坏或 bash 挂起。

**解法**：
- temp-scripts/中间文件写自己的 `temp-scripts/` 目录，用 **write_file 原生直写**（零 shell、零转义问题）
...
## 8. 日志时间用 UTC、文件 mtime 用本地（+0800）：跨文件判时间先对时区基准

**现象**：`_fish_loop.log` / `monitor-log.md` / `{角色名}_poll-log.md` 的时间戳（如 `[15:55:15]`）是 **UTC**，而产出文件/签字/退场文件的 mtime 是**本地时间（+0800）**——同一条流水线两套时钟基准，差 8 小时。

**案例**：2026-08-31 flow-test 批次。日志停在 `[16:24:01]` 而系统本地时间是 00:24——把日志时间当本地时间，误判"检测层已停"（实际活着：16:24 UTC = 本地 00:24，进程每 60s 正常写日志）；进而把本项目的 `_fish_loop.js` 进程误认成"别的项目的"，差点误导用户杀错进程/多余重启大鱼窗口。

**规避**：
- 判定进程存活/日志新鲜度：拿日志里 HH:MM:SS 和系统当前时间（`date` 本地）对齐前，**先确认日志的时区基准**（`_fish_loop.log` 的 `[HH:MM:SS]` 是 UTC；文件 mtime 是本地）
- 跨文件比时序：全部转 epoch（`stat -c %Y` 或 `fs.statSync().mtimeMs`）再比，别直接比格式化字符串
- 根治方向：脚本日志统一用本地时间或带时区标记（如 `+08:00`），避免两套基准混用
