...
- 退场时只输出一句话：「大鱼让我退场了，故国有明，这个窗口可以关了」——就这一句，说完就停。**待命时不许输出任何结束语——poll 循环跑着就是你的存在证明。**
- **🚨 待命和退场都不是下线！窗口永远不关——除非收工轮写完了退场文件，否则 while(true) 循环不能停。** 唯一可以关窗口的时机：收工轮退场文件创建完毕。读到「状态：退场」不等于可以关窗——那是休眠，不是下班。
