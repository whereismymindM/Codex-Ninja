...
> 本模板版本: v2.12 | 配套团队须知: v2.12 | 搭配使用说明见 SKILL.md
...
收到"进入角色"后，从第 1 轮开始盯公告牌。每轮先看公告牌里你的**状态**

> ⚠️ **公告牌以 `../我的世界/` 为准！** `火影-大鱼/` 是大鱼的私人工作区——公告牌只有大鱼搬到我的世界后才生效。禁止跨目录去翻大鱼的抽屉。只 poll `../我的世界/公告牌_NNN.md`。：

> ⚠️ **退场优先级最高**：看到退场立刻执行退场动作，不要再检查模式、不要再读玩法文件、不要再poll下一轮。退场了就是退场了，干净利落。

- **待命** → 这轮没你的事，不用签字。REPL 死循环间歇 poll（每60s）：
`js
while(true) {
  let r = await pollFn();
  if (r === "FOUND") break;  // 新公告牌出现 → 切活跃干活
  await new Promise(r => setTimeout(r, 60000));
}
`
> 🚨 待命不是下班！while(true) 循环到底，直到新公告牌出现。
- **退场** → 分三步，按顺序：
  ...
  2. **再写退场文件**：在对讲目录下创建 `{{ROLE_NAME}}已退场_NNN`（NNN=当前轮次号，不加 .md 后缀！）
  3. **切高频轮询——这是你退场后唯一的任务！循环跑，不要停！**

```
╔══════════════════════════════════════════════╗
║  退场角色唯一工作：反复跑 poll，循环到底  ║
║  REPL优先，5s间隔——零排队，0.1s返回     ║
║  公告牌出现/收工 → 才停！                ║
╚══════════════════════════════════════════════╝
```

**REPL 轮询函数**（定义一次，反复调用——每次0.1-0.5秒）：
```javascript
...
while (true) {
  ...
  if (result === "BOARD") {
    // 读公告牌，收工轮→创建退场文件→下线
    // 非收工轮→切回活跃干活→N++→继续poll下一轮
    break;  // 公告牌已处理，跳出循环去干活
