...
1. **状态**（这一轮干什么）→ `活跃`=干活、`待命`=这轮跳过、`休眠`=低功耗等下一轮
2. **本轮后**（干完活之后什么状态）→ `待命`=间歇poll等下一轮、`休眠`=写休眠文件→切低功耗poll

> ⚠️ **公告牌以 `../我的世界/` 为准！** `火影-大鱼/` 是大鱼的私人工作区——公告牌只有大鱼搬到我的世界后才生效。禁止跨目录去翻大鱼的抽屉。只 poll `../我的世界/公告牌_NNN.md`。

> ⚠️ **判断顺序**：先看状态（要不要干活），再看本轮后（干完活去哪）。状态=休眠 和 状态=活跃+本轮后=休眠 是两回事——前者直接休眠，后者先干活再休眠。
...
- **休眠** → 🚨 **休眠不是收工！休眠 = 挂机等下一轮公告牌，不是下班！** 只有公告牌写「模式：收工」才是真终点。休眠后唯一该做的事：跑 while(true) 高频 poll——等收工轮或唤醒，循环到底。
  分三步，按顺序：
  ...
  2. **再写休眠文件**：在对讲目录下创建 `{{ROLE_NAME}}已休眠_NNN`（NNN=当前轮次号，不加 .md 后缀！）。只写一次（除非被唤醒后重新进入休眠）。如果在 002 休眠了写 `已休眠_002`，003/004 延续休眠状态不再重复写——直到收工轮写 `已退场_NNN` 终局退场。
  3. **切高频轮询——这是你休眠后唯一的任务！N 从文件系统自动扫描，无需手动++。**
  在 REPL 中执行以下真实可执行代码（不是伪代码，直接复制粘贴运行）：

```js
// 休眠高频轮询——5s间隔等收工轮或唤醒。N从文件系统扫描，REPL超时重启也不丢进度
var _pollCount = 0;
...
```
> 🚨 **这不是伪代码。直接复制粘贴到REPL运行。** 休眠后唯一的工作就是跑这个while(true)循环——收工轮不到，循环不停。写完流水账和休眠文件后立刻跑，不要等、不要停。
...
- 休眠时只输出一句话：「大鱼让我休眠了，故国有明，这个窗口不要关」——就这一句，说完就停。**待命时不许输出任何结束语——poll 循环跑着就是你的存在证明。**
- **🚨 待命和休眠都不是下线！窗口永远不关——除非收工轮写完了退场文件，否则 while(true) 循环不能停。** 唯一可以关窗口的时机：收工轮退场文件创建完毕。读到「状态：休眠」不等于可以关窗——那是休眠，不是下班。
