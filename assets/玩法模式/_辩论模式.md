...
裁判等正反双方交总结陈词。**优先用 `wait_file.js` 多目标**（不带 `--any` = 双方都就位才返回）：`node 临时脚本/wait_file.js <任务目录>/辩论_06_正方总结.md <任务目录>/辩论_06_反方总结.md --hb <心跳> --timeout 20`（**`<任务目录>`/`<心跳>` 为占位符，复制后必须替换**）
...
裁判结论是文档类产出——先 writeFileSync 写到 `../我的世界/产出/` 目录，再 `node _deliver.js 裁判结论.md <任务目录>` 发信号，**然后 `node _sign.js N` 签字**（裁判是活跃角色，monitor 逐角色核对签字，漏签收工审计标红）。
...
- **本轮后：休眠** → 写流水账 → 创建休眠文件 → bash while + _reasonix_poll.js 轮询（每3s）——循环骨架见 `_干活流程.md`「休眠」节
