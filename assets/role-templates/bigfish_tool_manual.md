...
> **原则**：工具输出不理解时查本手册，**不要翻 monitor.js 源码**——本手册就是源码行为的完整说明。**例外**：判据拿不准某项会被怎么解析时，允许读 monitor.js 确认（文档写的是意图，代码是执行真相）。
> 关联：`bigfish_board_manual.md` 管公告牌校验/发布/扣留/追加；本手册管工具本身。
...
```bash
node ../monitor.js    # CWD=你的大鱼目录，world/ 在 monitor.js 同级
```

实际每 60s 由 `_fish_loop.js` 自动跑；你想手动看最新一轮，跑 `node _fish_loop.js --once` 即可。

### 24 种输出完整解读（22 主输出 + WARN/MONLOG_WARN 日志变体）
...
| `STUCK 角色` | 唤醒未确认+无产出=挂死（2026-08-22 分类） | **先查 poll-log 判进程**（`ls -la ../world/{角色名}_talk/{角色名}_poll-log.md`）：仍在更新=在轮询真挂死 → 写求助老渣（人工干预）；**已停=回合结束/窗口没了 → 只能用户唤醒，直接提醒用户**（写 `_wakeup.md` 无效——老渣也是 agent，同样不能替用户唤醒） |
...
| `WARN …` | 辅助告警（写唤醒文件失败/死锁保留提示等） | 查看详情，必要时人工介入 |
| `MONLOG_WARN` | monitor-log写入失败 | 记录，下次运行自动恢复 |

### 特殊行为

- **产出格式 A/B**：带文件名=逐文件查 `.ready`；仅目录（尾斜杠）=目录内任一 `.ready`
- **`产出负责人: 各自`** = 每个活跃角色需有带自己 `producer:` 的 `.ready`（monitor 校验归属——一人重复交付凑数判不过；历史无 producer 行按未知顶缺）
...
# 例：node _wakeup.js <角色名> <唤醒原因>
```

- 在 `world/{角色名}_talk/` 创建 `_wakeup.md`
- 角色检测后**删除** = ack；**补唤醒后 ≥20 分钟仍无推进（无 ack 且无产出）→ 写求助老渣**（干预阶梯对齐窗口常驻模板「项目启动」；run 形态同步——20 分钟无推进 → 求助，勿重复补写唤醒）；收口后清理残留
...
- 只用于老渣发到对讲目录的任务交付闭环（`fish_laozha_talk/`），不走公告牌 monitor
...
| 报 DEAD 但角色在干活 | 看同轮是否有 SKIP | 窗口常驻 2 分钟内 / run 拉起 10 分钟内新产出=活着，SKIP 不唤醒 |
