...
> **默认动作永远是：`node _sign.js N` / `node _deliver.js 文件 taskNNN` / `node _lock.js acquire|release`**。跳过本文件不影响任何日常干活。
>
...
var sign = async function(roundN) {
  ...
  var signFile = path.default.join(worldDir, roleName + "_talk", "done_" + Npad + ".md");
...
**回到 `_tool_guide.md`**（日常速查用）。签名即上述函数——需要定制逻辑时才读本文件手抄。
