...
| 大鱼模板 | `bigfish_AGENTS_template_window.md` | `bigfish_AGENTS_template_run.md` |
...
> 共享底座（公告牌格式、monitor、待命轮、扣留收工轮）两形态通用，见 `assets/operator-docs/board_complete_guide.md`。
> ⚠️ 但两形态的掉线处理不同：窗口常驻 = 角色心跳超时由 monitor 自动写唤醒文件，角色恢复轮询后感知并继续（系统有兜底，你不用担心）；run 拉起 = 角色干完即退、心跳停是正常态（不是掉线，别误判），唤醒由大鱼调度负责。
...
> ⚠️ **手动同步单文件模板必替换占位符**：不想删目录重建、只手动 cp 模板到角色目录时——**cp 是原样复制，scaffold 才会替换 `{{ROLE_NAME}}`**；不替换 = 心跳路径写进字面 `{{ROLE_NAME}}_talk/` → monitor 误判 DEAD。
...
角色窗口常驻后自动轮询公告牌推进（读牌 → 干活 → deliver+sign → 回 poll；非活跃轮自动跳过），**全程无人值守**，直到收工轮退场。机制细节见角色文档，你不需要管。
...
# 例：cd 项目根目录 && node <skill>/scripts/start-fish.js
```
...
角色初始不启动（或建会话后退出）→ 大鱼判断第 N 轮需要谁 → `reasonix run --continue/--dir` 唤醒 → 角色读公告牌 + 前置产出干活 → 干完即退（内存释放）→ 大鱼周期 monitor 检测完成 → 下一轮再唤醒。
...
**"永久轮询"不是字面无限循环**——角色在单会话内持续干活直到收工轮（scaffold 自动生成配置 + 回合保持协议，无需你干预），全程无人值守（实测单回合 20+ 分钟）。

**终止条件**：收工轮创建退场文件 → 回合结束（这是唯一正常退出）。

**边界（撞墙降级，不丢进度）**：`max-steps` 是硬墙。耗尽后 turn 被强制结束，但文件系统进度（output/签字/对话/退场）全部保留——重新 run 一次，角色按模板「启动自检」（签字存在→N++ 跳过已完成轮）从断点续接。
...
> run 模式**没有** `--gui` 参数。要看过程就用 `-show-thinking`；要记账就用 `-metrics`。
